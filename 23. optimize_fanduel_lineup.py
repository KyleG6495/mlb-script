...
print("🚀 FanDuel Lineup Optimizer - TODAY'S ACTUAL SLATE")

# 1. Load TODAY'S actual FanDuel slate
print("🔄 Loading TODAY'S FanDuel slate...")
...
print(f"📊 Today's slate: {len(todays_slate)} total players")
print(f"📊 Hitters: {len(todays_slate[todays_slate['Position'] != 'P'])} available")
print(f"📊 Pitchers: {len(todays_slate[todays_slate['Position'] == 'P'])} available")
...
print(f"\n💰 Salary range: ${all_players_today['Salary'].min()} - ${all_players_today['Salary'].max()}")
...
if 'Batting Order' in hitters_today.columns and hitters_today['Batting Order'].notna().any():
    ...
    print(f"\n✅ Found {len(hitter_starters)} confirmed hitter starters with batting orders")
else:
    print(f"\n⚠️ No batting order data - using salary-based starter filtering")
...
if 'Probable Pitcher' in pitchers_today.columns:
    ...
    print(f"✅ Found {len(pitcher_starters)} confirmed starting pitchers")
else:
...
if 'Injury Indicator' in starters.columns:
    ...
    if len(injured_players) > 0:
        ...
        for _, player in injured_players.iterrows():
            ...
            print(f"  ❌ {player['Nickname']} ({player['Team']}) - {player['Injury Indicator']} - {injury_detail}")
...
if 'Injury Details' in starters.columns:
    ...
    if len(il_players) > 0:
        ...
        for _, player in il_players.iterrows():
            print(f"  ❌ {player['Nickname']} - {player['Injury Details']}")
...
print(f"✅ Injury filter complete:")
...
if starters['FPPG'].isna().all():
    print("⚠️ All FPPG values are NaN - using salary-based projections")
...
print(f"✅ Created projections: {starters['Projected_FPPG'].min():.1f} - {starters['Projected_FPPG'].max():.1f}")

# 5. Show top players by game for verification
print(f"\n📋 TOP PLAYERS BY GAME (Salary-Based):")
...
print(f"\n📊 CORRECTED position distribution:")
...
print(f"\n🔧 Current assignment: {c_count} C players, {b1_count} 1B players")

# We need at least 1 C and 1 1B
if c_count < 1 or b1_count < 1:
    print("🔧 Rebalancing positions to ensure we have both C and 1B...")
    ...
    if len(dual_eligible) >= 2:  # Need at least 2 to split between C and 1B
        ...
        print(f"✅ Assigned to C: {top_for_c.iloc[0]['Nickname']} (${top_for_c.iloc[0]['Salary']})")
        print(f"✅ Assigned to 1B: {len(rest_for_1b)} players")
        
        # Show first few 1B assignments
        for _, player in rest_for_1b.head(3).iterrows():
            print(f"   {player['Nickname']} (${player['Salary']})")
    else:
        print("❌ Not enough dual-eligible C/1B players for proper split")

# Final verification
c_count_final = len(starters[starters['Primary_Position'] == 'C'])
b1_count_final = len(starters[starters['Primary_Position'] == '1B'])

print(f"\n📊 FINAL REBALANCED position distribution:")
print(starters['Primary_Position'].value_counts())
print(f"✅ Catchers: {c_count_final}, First Basemen: {b1_count_final}")
...
print(f"\n📊 Final lineup pool: {len(lineup_pool)} players")
...
if len(lineup_pool) < 9:
    print("❌ Not enough players for optimization!")
    exit()

# 8. Create optimization problem with STACKING
print("\n🎯 Setting up lineup optimization with STACKING STRATEGY...")
...
def optimize_with_stacking(lineup_pool, strategy='balanced'):
    ...
    for team in lineup_pool[lineup_pool['Primary_Position'] != 'P']['Team'].unique():
        ...
        if len(team_hitters) >= 3:
            ...
            print(f"  📊 {team}: {len(team_hitters)} eligible, {team_bonus:.1f} bonus points")
    ...
    for game in lineup_pool['Game'].unique():
        ...
        if len(game_hitters) >= 4:
            ...
            print(f"  🏟️ {game}: {len(game_hitters)} eligible, {game_bonus:.1f} bonus points")
    ...
    for pos, required in position_requirements.items():
        ...
        if len(pos_players) >= required:
            ...
            print(f"⚠️ Warning: Only {len(pos_players)} {pos} players available, need {required}")
...
print("🔄 Solving optimization with STACKING STRATEGY...")
...
if status == 1:  # Optimal solution found
    ...
    print(f"\n🏆 OPTIMAL STACKED LINEUP - TODAY'S ACTUAL SLATE:")
    ...
    print(f"🎯 Total Projected FPPG: {lineup['Projected_FPPG'].sum():.2f}")
    ...
    if len(team_stacks) > 0:
        ...
        print(f"📊 No team stacks (2+ players) in lineup")
    ...
    if active_team_stacks:
        print(f"  🏟️ Team stacks: {', '.join(active_team_stacks)}")
    
    active_game_stacks = [game for game, var in game_stack_vars.items() if var.value() == 1]
    if active_game_stacks:
        print(f"  🎯 Game stacks: {', '.join(active_game_stacks)}")
    
    if not active_team_stacks and not active_game_stacks:
        print(f"  📊 No major stacking bonuses activated (pure value lineup)")
    ...
    print(f"📈 Correlation Score: {correlation_score} (higher = more correlated)")
    ...
    print(f"✅ Saved STACKED lineup to {output_file}")
    ...
    print("❌ No optimal solution found with stacking constraints.")
