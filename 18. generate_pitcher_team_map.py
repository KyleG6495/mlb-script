...
def main():
    ...
    logging.info(f"SUCCESS: Loaded {len(df_stats)} pitcherfeature rows")
    ...
    if "Position" in df_slate.columns:
        ...
        logging.info(f" Filtered df_slate to {len(df_slate)} pitcher rows")
    ...
    if "Position" in df_stats.columns:
        ...
        logging.info(f" Filtered df_stats to {len(df_stats)} pitcher rows")
