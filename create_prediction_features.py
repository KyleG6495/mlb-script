...
fd_slate = pd.read_csv(FilePaths.FD_HITTER_FEATURES_FINAL)
...
historical_features = pd.read_csv(FilePaths.HITTER_ROLLING_5GAME_FEATURES)
...
final_prediction_features.to_csv(FilePaths.PREDICTION_READY_FEATURES, index=False)
print(f"\nSUCCESS: Saved prediction-ready features to: {FilePaths.PREDICTION_READY_FEATURES}")
...
try:
    ...
    if os.path.exists(FilePaths.TOTAL_BASES_PIPELINE):
        model = joblib.load(FilePaths.TOTAL_BASES_PIPELINE)
        ...
        print(f"   Model not found at: {FilePaths.TOTAL_BASES_PIPELINE}")
