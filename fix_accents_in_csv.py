...
def fix_csv_accents(input_file, output_file=None):
    ...
    print(f"STEP: Fixing accents in: {input_file}")
    ...
    try:
        ...
        for encoding in ['utf-8', 'latin-1', 'cp1252']:
            try:
                ...
                print(f"SUCCESS: Successfully read with {encoding} encoding")
                break
        ...
        if 'name' in df.columns:
            ...
            problematic_names = df[df['name'].str.contains('|||||||', na=False)]['name'].head(5)
            ...
            print(f"SUCCESS: Fixed {fixed_count} player names")
        else:
            print("WARNING: No 'name' column found in CSV")
...
def main():
    ...
    print("STEP: ACCENT FIXER FOR CSV FILES")
    print("=" * 40)
    ...
    for filename in files_to_fix:
        ...
        if file_path.exists():
            ...
            if success:
                print(f"SUCCESS: {filename} fixed successfully!")
            else:
                print(f"ERROR: Failed to fix {filename}")
        else:
            print(f"WARNING: File not found: {filename}")
    
    print("\nCOMPLETE: Accent fixing complete!")
