...
def main():
    # 1) Load seasonlevel features (now fd_hitter_features_enriched.csv)
    ...
    for df, name in [(season, "season"), (rolling, "rolling5"), (daily, "daily")]:
        if "date" in df.columns:
            logging.info(f"SWAP: Coercing {name}.date to datetime")
    ...
    if 'player_id' in common_daily_rolling and 'date' in common_daily_rolling:
        ...
        logging.error("ERROR: No common merge keys found between daily and rolling data")
        return
