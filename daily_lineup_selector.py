...
def analyze_daily_lineups():
    ...
    print("TARGET: DAILY ENHANCED LINEUP SELECTOR")
    print("=" * 50)
    ...
    if not lineup_files:
        print("ERROR: No enhanced lineups found!")
        return
    ...
    print("\nLINEUP: DAILY LINEUP RECOMMENDATIONS")
    print("=" * 50)
    
    # Cash Game Recommendation
    if 'balanced' in recommendations:
        cash_lineup = recommendations['balanced']
        print(f"\nMONEY: CASH GAMES (50/50s, Double-Ups):")
        print(f"    Use: {cash_lineup['file']}")
        print(f"    Salary: ${cash_lineup['total_salary']:,.0f}")
        print(f"   PROGRESS: Projection: {cash_lineup['total_projection']:.1f} FPPG")
        print(f"   TARGET: Floor Score: {cash_lineup['floor_score']:.1f} (conservative)")
    ...
    if 'gpp' in recommendations:
        ...
        print(f"   PROGRESS: Projection: {gpp_lineup['total_projection']:.1f} FPPG")
        print(f"   START: Ceiling Score: {gpp_lineup['ceiling_score']:.1f} (upside)")
    ...
    if 'gpp' in lineups and len(lineups['gpp']) >= 3:
        ...
        print(f"   TIP: Strategy: Different stacks/pivots for tournament coverage")
    
    if 'balanced' in lineups and len(lineups['balanced']) >= 2:
        print(f"   MONEY: For Cash Multi-Entry (2-3 lineups):")
        for i, lineup in enumerate(lineups['balanced'][:2]):
            print(f"      #{i+1}: {lineup['file']} ({lineup['total_projection']:.1f} FPPG)")
        print(f"   TIP: Strategy: Slight variations for cash game security")
    
    # Weather impact analysis
    print(f"\n WEATHER & PARK FACTORS ACTIVE:")
