...
def get_pitcher_game_logs(player_id):
    url = f"https://statsapi.mlb.com/api/v1/people/{player_id}/stats?stats=gameLog&group=pitching&sportId=1&season=2025"
    try:
...
for _, row in tqdm(pitchers.iterrows(), total=len(pitchers), desc="Fetching pitcher logs"):
    ...
    game_logs = get_pitcher_game_logs(player_id)
    if not game_logs:
        continue
