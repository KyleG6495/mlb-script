...
echo.
echo Prerequisites:
echo   ✅ fd_slate_today.csv downloaded from FanDuel
echo   ✅ starting_lineups.csv created
echo.

:retry_clean_slate
echo Step 0: Cleaning slate data (removing injured players)...
"%PYTHON_EXE%" CLEAN_SLATE_DATA.py
call "%DAILY_RUNNERS_DIR%error_handler.bat" "Clean Slate Data" %errorlevel%
if errorlevel 999 goto :retry_clean_slate
if errorlevel 1 goto :error
echo ✅ Slate cleaned - removed injured players
echo.
echo Press any key to continue with confirmed starters data processing...
pause
echo.
if errorlevel 1 goto error
