...
available_columns = fd_slate.columns.tolist()
print(f"   Available columns: {available_columns[:10]}...")  # Show first 10 columns

# Build base_columns dynamically based on what's available
base_columns = [player_id_col]  # Start with player ID column

# Add name columns if available
if 'First Name' in available_columns:
    base_columns.append('First Name')
if 'Last Name' in available_columns:
    base_columns.append('Last Name')
elif 'Nickname' in available_columns:
    base_columns.append('Nickname')

# Add team/opponent columns if available  
if 'Team' in available_columns:
    base_columns.append('Team')
if 'Opponent' in available_columns:
    base_columns.append('Opponent')

print(f"   Using base columns: {base_columns}")

# Start with FD slate - use only available columns
enhanced = fd_slate[base_columns].copy()

# Create name column based on available columns
if 'First Name' in base_columns and 'Last Name' in base_columns:
    enhanced['name'] = enhanced['First Name'] + ' ' + enhanced['Last Name']
elif 'Nickname' in base_columns:
    enhanced['name'] = enhanced['Nickname']
else:
    enhanced['name'] = 'Unknown Player'
...
for col in numeric_cols:
    if col in enhanced.columns:
        # Convert to numeric first, replacing any non-numeric values with 0
...
# Add team encoding
if 'Team' in enhanced.columns and 'Opponent' in enhanced.columns:
    ...
    print("   Team/Opponent columns not available - using default values")
    enhanced['team'] = 0
    enhanced['opponent'] = 0
