...
from config import FilePaths, MLBApiConfig, LoggingConfig
...
# Fetch all active MLB players once with proper error handling
url = f"{MLBApiConfig.BASE_URL}/sports/1/players"
logging.info("🔄 Fetching active MLB player list from API...")

try:
    res = requests.get(url, timeout=MLBApiConfig.TIMEOUT)
    res.raise_for_status()
    players = res.json().get("people", [])
    logging.info("✅ Retrieved %d active MLB players", len(players))
except requests.exceptions.RequestException as e:
    logging.error("❌ Failed to fetch MLB players: %s", e)
    players = []
...
# Apply manual overrides from centralized configuration
