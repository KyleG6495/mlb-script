...
if not os.path.exists(PROJ_PATH):
    raise FileNotFoundError(f"Couldn’t find projections at {PROJ_PATH}")
...
logging.info(f"✅ Loaded projections: {len(proj)} unique player_ids")

# 2) Load the raw FanDuel slate
if not os.path.exists(SLATE_PATH):
    raise FileNotFoundError(f"Couldn’t find slate at {SLATE_PATH}")
slate = pd.read_csv(SLATE_PATH, dtype=str)
logging.info(f"✅ Loaded raw FD slate: {len(slate)} rows; columns: {list(slate.columns)}")
...
logging.info(f"ℹ️ Parsed {slate['player_id'].nunique()} unique player_ids in slate")
...
if len(missing_ids):
    logging.warning("⚠️ These player_ids have no proj_points:")
...
logging.info(f"✅ Saved enriched FD slate → {OUTPUT_PATH}")
