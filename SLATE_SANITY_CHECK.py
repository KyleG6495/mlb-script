...
def analyze_viable_players():
    ...
    print(f"DATA: Total players in slate: {len(df)}")
    ...
    if len(injured) > 0:
        print(f"  MONEY: Total salary of injured: ${injured['Salary'].sum():,}")
    ...
    print(f"\nSUCCESS: HEALTHY PLAYERS:")
    print(f"   Healthy players: {len(healthy)}")
    print(f"  MONEY: Available salary pool: ${healthy['Salary'].sum():,}")
    ...
    print(f"  ERROR: Non-probable pitchers: {len(pitchers) - len(probable_pitchers)}")
    
    if len(probable_pitchers) > 0:
        print(f"  MONEY: Probable pitcher salary range: ${probable_pitchers['Salary'].min():,} - ${probable_pitchers['Salary'].max():,}")
        print(f"  PROGRESS: Probable pitcher FPPG range: {probable_pitchers['FPPG'].min():.1f} - {probable_pitchers['FPPG'].max():.1f}")
        
        print(f"\nTARGET: TOP 10 PROBABLE PITCHERS:")
    ...
    print(f"\n HITTER ANALYSIS:")
    print(f"  OWNERSHIP: Total healthy hitters: {len(hitters)}")
    print(f"  MONEY: Hitter salary range: ${hitters['Salary'].min():,} - ${hitters['Salary'].max():,}")
    print(f"  PROGRESS: Hitter FPPG range: {hitters['FPPG'].min():.1f} - {hitters['FPPG'].max():.1f}")
    
    # Check for batting order availability
    hitters_with_order = hitters[hitters['Batting Order'].notna()]
    print(f"  INFO: Hitters with batting order: {len(hitters_with_order)}")
    print(f"   Hitters without batting order: {len(hitters) - len(hitters_with_order)}")
    
    print(f"\nTARGET: TOP 15 VALUE HITTERS:")
    ...
    for _, h in top_ceiling.iterrows():
        ...
        pos = h['Roster Position']
        matchup = f"{h['Team']} vs {h['Opponent']}"
        print(f"    START: {name:20} ({pos:4}) ${h['Salary']:4,} | {h['FPPG']:5.1f} FPPG | {h['value']:4.1f} val | {matchup}")
    
    # Position analysis
    print(f"\nDATA: POSITION AVAILABILITY:")
    for pos in ['P', 'C', '1B', '2B', '3B', 'SS', 'OF']:
        ...
        if len(pos_players) > 0:
            ...
            print(f"  {pos:2}: {len(pos_players):3} players | ERROR: PROBLEM!")
    
    print(f"\nTIP: RECOMMENDATIONS:")
    print(f"  1. SUCCESS: Use ONLY healthy players (remove {len(injured)} injured)")
    print(f"  2. BASEBALL: Use ONLY probable pitchers ({len(probable_pitchers)} available)")
    print(f"  3. TARGET: Focus on value plays (4.0+ value score)")
    print(f"  4. START: Mix in ceiling plays (15+ FPPG)")
    print(f"  5. INFO: Wait for batting orders if not available")
...
if __name__ == "__main__":
    ...
    print(f"\nDATA: SUMMARY: {stats['healthy_players']} viable players out of {stats['total_players']} total")
