...
class GameStateEnhancedDFS:
    ...
    def enhance_real_projections(self, fd_hitter_path: str = None, fd_pitcher_path: str = None, n_simulations: int = 2000):
        ...
        # Load position data from starting lineups
        starting_lineups_path = os.path.join(os.path.dirname(fd_hitter_path), 'starting_lineups.csv')
        try:
            position_data = pd.read_csv(starting_lineups_path)
            logger.info(f"Loaded position data for {len(position_data)} players from starting_lineups.csv")
        except FileNotFoundError:
            logger.error(f"Starting lineups file not found: {starting_lineups_path}")
            return None
        ...
        try:
            ...
            # Merge position data with hitters
            # Use player name matching - handle both name formats
            if 'target_name_x' in hitter_data.columns:
                hitter_data['player_name'] = hitter_data['target_name_x']
            elif 'First Name' in hitter_data.columns and 'Last Name' in hitter_data.columns:
                hitter_data['player_name'] = hitter_data['First Name'] + ' ' + hitter_data['Last Name']
            elif 'first_name' in hitter_data.columns and 'last_name' in hitter_data.columns:
                hitter_data['player_name'] = hitter_data['first_name'] + ' ' + hitter_data['last_name']
            elif 'Name' in hitter_data.columns:
                hitter_data['player_name'] = hitter_data['Name']
            elif 'name' in hitter_data.columns:
                hitter_data['player_name'] = hitter_data['name']
            else:
                logger.error("Could not find name column in hitter data")
                return None
            
            # Merge position data
            hitter_data = hitter_data.merge(
                position_data[['player_name', 'position']], 
                on='player_name', 
                how='left'
            )
            hitter_data['Position'] = hitter_data['position']
            logger.info(f"Merged position data - {len(hitter_data[hitter_data['Position'].notna()])} hitters have positions")
        ...
        try:
            ...
            # Add Position column for pitchers
            pitcher_data['Position'] = 'P'
    ...
    def _enhance_hitters(self, hitter_data: pd.DataFrame, n_simulations: int, weather_data: dict) -> list:
        ...
        for idx, player in hitter_data.iterrows():
            # Create player data for simulation - handle different column name formats
            if 'First Name' in player.index and 'Last Name' in player.index:
                player_name = f"{player['First Name']} {player['Last Name']}"
            elif 'Nickname' in player.index:
            ...
            player_data = {
                'id': f"{player.get('Team', 'UNK')}_{idx}",
                'name': player_name,
                'position': player.get('Position', 'UNK'),
                'team': player.get('Team', 'UNK'),
                'salary': player.get('Salary', 3000),
            ...
            enhanced_hitters.append({
                'First Name': player.get('First Name', player_name.split()[0] if ' ' in player_name else player_name),
                'Last Name': player.get('Last Name', player_name.split()[-1] if ' ' in player_name else ''),
                'Position': player.get('Position', 'UNK'),
                'Team': player.get('Team', 'UNK'),
                'Salary': player.get('Salary', 3000),
                'FPPG': enhanced_fppg,
    ...
    def _enhance_pitchers(self, pitcher_data: pd.DataFrame, n_simulations: int, weather_data: dict) -> list:
        ...
        for idx, player in pitcher_data.iterrows():
            ...
            # Handle different column name formats for pitcher names
            if 'First Name' in player.index and 'Last Name' in player.index:
                pitcher_name = f"{player['First Name']} {player['Last Name']}"
            elif 'Nickname' in player.index:
                pitcher_name = player['Nickname']
            elif 'name' in player.index:
                pitcher_name = player['name']
            else:
                pitcher_name = f"Pitcher_{idx}"
            ...
            enhanced_pitchers.append({
                'First Name': player.get('First Name', pitcher_name.split()[0] if ' ' in pitcher_name else pitcher_name),
                'Last Name': player.get('Last Name', pitcher_name.split()[-1] if ' ' in pitcher_name else ''),
