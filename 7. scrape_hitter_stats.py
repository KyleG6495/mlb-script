...
logging.info(f"INFO: Found {len(hitters)} unique hitters to scrape")
...
if all_logs:
    ...
    logging.info(f"SUCCESS: Saved {len(out_df)} rows  {OUTPUT_FILE}")
else:
    logging.error("WARNING: No data scraped. Check debug logs for URL and response issues.")
