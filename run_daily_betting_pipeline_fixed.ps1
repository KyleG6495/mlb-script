...
if (-not $modelsExist -or $RetrainModels) {
    ...
    if ($TuneModels) {
        Write-Host "⚙️ Running with hyperparameter tuning..." -ForegroundColor Magenta
...
Write-Host "`n🎯 Analyzing betting opportunities..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $reportDir) {
        ...
        if ($latestReport) {
            Write-Host "`n📊 LATEST BETTING REPORT:" -ForegroundColor Magenta
            Write-Host "-" * 40
