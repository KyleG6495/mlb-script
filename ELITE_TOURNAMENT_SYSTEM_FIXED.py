#!/usr/bin/env python3
"""
ELITE TOURNAMENT SYSTEM ENHANCED - FIXED
Based on 104-point, top 35% finish analysis with current slate validation
Incorporates Hurston Waldrep success patterns using TODAY'S active players
"""
...
class EliteTournamentSystem:
    ...
    def identify_waldrep_patterns(self, players_df, ownership_df):
        """Find pitchers matching Hurston Waldrep success pattern - ACTIVE PLAYERS ONLY"""
        logger.info("🎯 IDENTIFYING WALDREP-TYPE OPPORTUNITIES (Active Players)")
        ...
        try:
            ...
            if 'Injury Indicator' in enhanced_df.columns:
                active_players = enhanced_df[enhanced_df['Injury Indicator'].isna()]
                injured_count = len(enhanced_df) - len(active_players)
                if injured_count > 0:
                    logger.info(f"🚨 Filtered out {injured_count} injured players")
                enhanced_df = active_players
            ...
            if not waldrep_candidates.empty:
                ...
                logger.info(f"🔥 FOUND {len(waldrep_candidates)} ACTIVE WALDREP-TYPE CANDIDATES:")
                ...
                logger.info("No Waldrep-type candidates found in active player pool")
    ...
    def identify_game_stack_opportunities(self, players_df):
        """Find elite game stacking opportunities from active players"""
        logger.info("⚡ IDENTIFYING ELITE GAME STACK OPPORTUNITIES (Active Players)")
        
        try:
            # Filter active players only
            if 'Injury Indicator' in players_df.columns:
                active_players = players_df[players_df['Injury Indicator'].isna()]
            else:
                active_players = players_df
                
            hitters = active_players[active_players['Position'] != 'P'].copy()
...
def main():
    logger.info("🏆 ELITE TOURNAMENT SYSTEM ENHANCED - FIXED")
    logger.info("=" * 60)
    logger.info("🎯 Based on 104-point, top 35% finish analysis")
    logger.info("⚡ Using TODAY'S active players only (no injured players)")
    logger.info("=" * 60)
    
    try:
        # Load latest data - DYNAMIC FILE DISCOVERY (NO HARDCODED DATES)
        logger.info("🔍 Finding most recent data files...")
        data_files = get_data_files()
        
        # Prioritize today's actual slate to avoid injured players
        if 'slate' in data_files:
            players_df = safe_read_csv(data_files['slate'])
        ...
        elif 'projections' in data_files and 'ownership' in data_files:
            players_df = safe_read_csv(data_files['projections'])
            ownership_df = safe_read_csv(data_files['ownership'])
            logger.info(f"✅ Loaded {len(players_df)} players and ownership data")
            logger.info("⚠️  Using fallback projections - may include injured players")
        ...
        waldrep_anchors = elite_system.identify_waldrep_patterns(players_df, ownership_df)
        game_stacks = elite_system.identify_game_stack_opportunities(players_df)
        ...
        if not waldrep_anchors.empty:
            ...
            logger.info("⚠️  No viable anchor candidates found in active player pool")
        ...
        logger.info(f"\n🚀 ELITE SYSTEM READY!")
        logger.info(f"✅ Active player validation complete")
        logger.info(f"✅ Injury filtering applied")
        logger.info(f"✅ Elite patterns identified from healthy players")
