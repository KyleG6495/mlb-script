...
ID_MAP_FILE = "../data/pitcher_games_with_game_pk.csv"
...
if bieber_in_merged.empty:
    ...
    if name_col and name_col in merged_df.columns:
...
# Remove duplicates by player_id and date (if date exists)
logging.info(f"Rows before removing duplicates: {len(merged_df)}")
logging.info(f"Available columns in merged_df: {merged_df.columns.tolist()}")

# Check which date column exists and use it
if 'date' in merged_df.columns:
    date_col = 'date'
elif 'date_rolling' in merged_df.columns:
    date_col = 'date_rolling'
elif 'date_season' in merged_df.columns:
    date_col = 'date_season'
else:
    date_col = None
    logging.warning("No date column found in merged_df")

if date_col:
    logging.info(f"Unique dates in merged_df: {sorted(merged_df[date_col].dropna().unique().tolist())[:10]} (first 10)")
else:
    logging.info("No date column available for date analysis")

# Check if date column exists, use appropriate subset for deduplication
if 'date' in merged_df.columns:
    duplicate_subset = ['player_id', 'date']
elif 'date_rolling' in merged_df.columns:
    duplicate_subset = ['player_id', 'date_rolling']
elif 'date_season' in merged_df.columns:
    duplicate_subset = ['player_id', 'date_season']
else:
    duplicate_subset = ['player_id']
    logging.warning("No date column found, deduplicating by player_id only")

duplicates = merged_df.duplicated(subset=duplicate_subset).sum()
if duplicates > 0:
    logging.warning(f"Duplicate rows in merged_df: {duplicates}")
    logging.info(f"Sample duplicates:\n{merged_df[merged_df.duplicated(subset=duplicate_subset, keep=False)][duplicate_subset].head().to_string()}")
merged_df = merged_df.drop_duplicates(subset=duplicate_subset)
...
# Deduplicate merged_df after team merge using the same logic as before
if 'date' in merged_df.columns:
    duplicate_subset_team = ['player_id', 'date']
elif 'date_rolling' in merged_df.columns:
    duplicate_subset_team = ['player_id', 'date_rolling']
elif 'date_season' in merged_df.columns:
    duplicate_subset_team = ['player_id', 'date_season']
else:
    duplicate_subset_team = ['player_id']
    logging.warning("No date column found for post-team merge deduplication")

duplicates_after_team = merged_df.duplicated(subset=duplicate_subset_team).sum()
if duplicates_after_team > 0:
    logging.warning(f"Duplicate rows after team merge: {duplicates_after_team}")
    logging.info(f"Sample duplicates:\n{merged_df[merged_df.duplicated(subset=duplicate_subset_team, keep=False)][duplicate_subset_team].head().to_string()}")
merged_df = merged_df.drop_duplicates(subset=duplicate_subset_team)
...
if date_col:
    ...
    logging.info("No date column available for date debugging")
...
if date_col:
    ...
    logging.info(f"Rows after filtering NaN {date_col}: {len(merged_df)}")
else:
    logging.info("No date column to filter, keeping all rows")
