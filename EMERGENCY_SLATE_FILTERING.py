...
def fix_slate_filtering():
    ...
    print(f"SUCCESS: Removed IL players: {len(slate_df)} players remaining")
    
    # Remove non-probable pitchers
    slate_df = slate_df[~((slate_df['Position'] == 'P') & (slate_df['Probable Pitcher'].isna()))]
    print(f"SUCCESS: Removed non-probable pitchers: {len(slate_df)} players remaining")
    
    # Remove players with 0 or negative projections
    slate_df = slate_df[slate_df['FPPG'] > 0]
    print(f"SUCCESS: Removed 0/negative projections: {len(slate_df)} players remaining")
    ...
    print(f"\nSUCCESS: Saved filtered slate: FILTERED_CORRECTED_slate_{timestamp}.csv")
    ...
    for _, player in top_players.iterrows():
        name = f"{player['First Name']} {player['Last Name']}"
        probable = "SUCCESS:" if pd.notna(player['Probable Pitcher']) else "ERROR:"
